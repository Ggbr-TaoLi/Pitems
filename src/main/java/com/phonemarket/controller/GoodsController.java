...
//	}
	...
	public Object uploadGoodsImg(@PathVariable(value="file")MultipartFile file,HttpServletRequest request){
		...
		String name=UUIDUtil.getUUID()+str.substring(str.lastIndexOf(".")); //部分代码在UUIDUtil中实现
...
//		return obj;
		...
		List<Map<String,Object>> list = orderService.findPopularGoods(5);
		String[] name=new String[5];
		Integer[] volume=new Integer[5];
		int i = 0;
		for (Map<String, Object> map : list) {
			Integer hotId = (Integer) map.get("hotId");
			Integer hotNums=Integer.valueOf(map.get("hotNums").toString());
			Goods g = goodsService.findById(hotId);
			name[i]=g.getGoodsName();
			volume[i]= hotNums;
			i++;
		}
		JSONObject obj=new JSONObject();
